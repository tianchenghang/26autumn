...
>
> 使用说明：本文档按「架构设计 → 进程通信 → Agent 核心 → 权限安全 → 终端 UI → 会话状态 → 扩展集成 → 工程化 → 开放权衡」九个维度组织，共 58 问。回答中大量引用源码位置（`文件:行号`），面试前建议对照源码通读。
...
> 文档版本：基于 `apps/swifty-code` 当前源码（2026-07）编写，共 58 问。引用行号以源码为准，代码演进后请以 `git log` 对照更新。
