...
1.  不定高支持：预估高度渲染，ResizeObserver 实测真实高度，动态修正 totalHeight 和偏移量。缓存已测高度，避免重复计算。
...
4.  收益量化：上线后 LLM patch 的自动验证通过率约 65%，人工审核后合入率约 47%，错误 MTTR 从约 4.2 天降至 0.8 天，每周节省 on-call 人力约 6 小时，释放人力投入复杂问题。
...
4.  Vite 坑点：CSS 隔离不完善；早期版本 dev 模式下远程模块 HMR 不生效（我提交 PR 在 handleHotUpdate 中修复了虚拟模块 invalidate）；生产构建需额外配置 legacy 兼容。大型项目建议仍用 Webpack MF。
