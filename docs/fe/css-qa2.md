...
A: CSS 选择器优先级是浏览器决定当多个规则作用于同一元素时, 哪个规则的样式生效的机制. 优先级由选择器的特异性 (Specificity) 决定, 特异性用一个四元组 (a, b, c, d) 表示.

特异性计算规则如下:

- d 位: 内联样式 (style 属性), 计为 1, 否则为 0
- a 位: ID 选择器的数量, 如 #header 计为 (1, 0, 0, 0)
- b 位: 类选择器、属性选择器、伪类的数量, 如 .class、[type="text"]、:hover 各计为 (0, 1, 0, 0)
- c 位: 元素选择器、伪元素的数量, 如 div、::before 各计为 (0, 0, 1, 0)
- 通配符 \*、子选择器 >、相邻选择器 +、通用兄弟选择器 ~ 不影响特异性

比较优先级时, 从左到右逐位比较, 高位优先. 例如:

- #nav .item > a:hover 的特异性为 (1, 2, 1, 1) — 1 个 ID, 1 个类 + 1 个伪类, 1 个元素
- .nav .item a 的特异性为 (0, 2, 1, 0) — 2 个类, 1 个元素
- 内联样式的特异性为 (1, 0, 0, 0, 0), 始终高于任何选择器组合
...
- Grid gap: 所有现代浏览器, IE 支持 -ms-grid-row-gap / -ms-grid-column-gap
- Flex gap: IE 不支持, 旧版 Safari 不支持
