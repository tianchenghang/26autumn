...
- 三、项目经历相关问题
  - [3.1 Sentry SDK - 架构设计](#31-sentry-sdk---架构设计)
  - [3.2 Sentry SDK - JSError 捕获与 Source Map](#32-sentry-sdk---jserror-捕获与-source-map)
  - [3.3 Sentry SDK - 错误去重与 LRU](#33-sentry-sdk---错误去重与-lru)
  - [3.4 Sentry SDK - 白屏检测](#34-sentry-sdk---白屏检测)
  - [3.5 Sentry SDK - 性能指标采集](#35-sentry-sdk---性能指标采集)
  - [3.6 Sentry SDK - rrweb 故障现场重放](#36-sentry-sdk---rrweb-故障现场重放)
  - [3.7 Sentry SDK - 数据上报降级策略](#37-sentry-sdk---数据上报降级策略)
  - [3.8 Sentry SDK - 包体积优化](#38-sentry-sdk---包体积优化)
  - [3.9 CLI Coding Agent - 架构设计](#39-cli-coding-agent---架构设计)
  - [3.10 CLI Coding Agent - Agent Loop 与 ReAct](#310-cli-coding-agent---agent-loop-与-react)
  - [3.11 CLI Coding Agent - 权限系统](#311-cli-coding-agent---权限系统)
  - [3.12 CLI Coding Agent - 上下文压缩](#312-cli-coding-agent---上下文压缩)
  - [3.13 CLI Coding Agent - 记忆系统](#313-cli-coding-agent---记忆系统)
  - [3.14 CLI Coding Agent - Subagent 与 Agent Team](#314-cli-coding-agent---subagent-与-agent-team)
  - [3.15 CLI Coding Agent - MCP 接入](#315-cli-coding-agent---mcp-接入)
...
swifty-sentry 采用发布订阅架构，分为 core 和 plugins 两层:

core 模块的核心能力:

1. EventBus (事件总线): 提供 on / off / emit 的发布订阅机制，是所有模块间通信的基础
2. 生命周期管理: SDK 初始化、配置合并、全局状态管理 (如用户信息、环境标签)
3. 数据管道 (Pipeline): 定义数据从采集到上报的处理流水线 (beforeSend hooks)
4. 基础类型定义: 错误事件、性能事件、用户行为等的统一类型

plugin 子模块:

1. 错误捕获 plugin: 订阅全局错误事件，捕获并标准化各类错误
2. 性能采集 plugin: 订阅 PerformanceObserver，采集 Web Vitals 指标
3. 白屏检测 plugin: 订阅路由变化事件，在页面加载完成后执行检测
4. rrweb 录制 plugin: 订阅 DOM 变化，录制屏幕快照
5. 上报 plugin: 订阅数据管道输出，执行降级上报策略

plugin 注册和生命周期:

- init({ plugins: [errorPlugin, perfPlugin, rrwebPlugin] }) 时注册
- 每个 plugin 实现 install(core) 接口，在 install 中订阅 core 的事件
- plugin 可以在 install 中注册自己的 hooks (如 beforeSend)
- SDK 销毁时调用每个 plugin 的 destroy() 方法，取消订阅、清理资源

架构优势:

1. 按需引入: 用户可以根据需要选择 plugin，减小包体积
2. 独立迭代: 各 plugin 可以独立发版和更新，互不影响
3. 可扩展: 用户可以编写自定义 plugin，扩展 SDK 能力
4. 可测试: 每个 plugin 可以独立单测，core 和 plugin 的交互通过 EventBus 的接口验证

### 3.2 Sentry SDK - JSError 捕获与 Source Map

前端 JSError 捕获方式:

1. window.onerror: 捕获同步运行时错误。签名: (message, source, lineno, colno, error) -> boolean。可以获取错误堆栈 (error.stack)，但无法捕获资源加载错误。

2. window.addEventListener('error'): 可以捕获资源加载错误 (img/script/link 的加载失败)。但捕获到的 event 是 Event 对象而非 Error 对象，没有堆栈信息，只有 target.src 等资源信息。

3. window.addEventListener('unhandledrejection'): 捕获未处理的 Promise rejection。event.reason 包含拒绝的原因。

4. try-catch: 最精确的捕获方式，但只能包裹同步代码。async/await 中的错误也可以用 try-catch 捕获。

5. React Error Boundary: 捕获 React 组件树中的渲染错误，通过 componentDidCatch 获取。

获取错误堆栈: error.stack 包含了完整的调用栈信息 (函数名、文件名、行号、列号)，但是生产环境中文件是压缩后的，行号列号对应的是 bundle 文件的坐标。

Source Map 还原流程:

1. 前端上报错误时，附带压缩后的堆栈信息 (文件名、行号、列号)
2. 服务端收到错误后，根据文件名和行号列号找到对应的 Source Map 文件
3. 使用 source-map 库 (Mozilla 的 vlq 解码 + mappings 解析) 将压缩后的坐标映射为源码坐标
4. 还原后的堆栈包含原始文件名、行号、列号和函数名

生产环境 Source Map 安全管理:

1. Source Map 不部署到 CDN / 生产服务器 (构建时生成但不上传到 Web 服务器)
2. 将 Source Map 上传到专门的 Source Map 管理服务 (如 Sentry 的 Release 管理)
3. 或者将 Source Map 放在内网 OSS，监控服务通过内网访问
4. 通过 webpack 配置 devtool: 'hidden-source-map' 生成 Source Map 但不在 bundle 中引用 (不添加 //# sourceMappingURL 注释)

### 3.3 Sentry SDK - 错误去重与 LRU

错误去重策略:

我采用的是基于指纹 (fingerprint) 的去重方式，而不是简单的消息比较:

1. 计算错误指纹: 综合考虑错误类型 (type)、错误消息 (message) 和堆栈帧 (stack frames 的前 3 帧的文件名 + 行号 + 函数名)
2. 对指纹做哈希 (如简单的字符串哈希)，作为去重 key
3. 相同 key 的错误在一定时间窗口内只上报一次

为什么不只基于消息: 同一个错误可能因为动态数据 (如 "Cannot read property 'x' of undefined" 中的 x 可能是不同属性名) 产生不同的消息，但实际上是同一类问题。基于堆栈帧的去重更准确。

为什么不只基于堆栈: 同一个位置可能抛出不同类型的错误，需要区分。

LRU 缓存的作用:

1. 维护最近上报过的错误指纹集合，用于去重判断
2. LRU 策略确保缓存大小有限 (如最多 1000 条)，超出时淘汰最久未命中的条目
3. 避免内存无限增长

上报频率控制 (防错误风暴):

1. 采样率: 全局配置 sampleRate (如 0.1 = 10% 采样)
2. 速率限制: 同一个指纹在 N 秒内最多上报 M 次
3. 队列+批量上报: 错误先进入队列，定时批量发送，而不是每个错误单独发请求
4. 熔断机制: 如果短时间内错误数量超过阈值 (如 100 个/10s)，暂停上报并告警

### 3.4 Sentry SDK - 白屏检测

关键点采样原理:

1. 在页面加载完成后 (DOMContentLoaded + 延迟一段时间)，对页面可视区域进行采样
2. 采样方式: 在页面的关键位置 (如页面中心、四角、以及根据页面布局预定义的 "关键点") 调用 document.elementsFromPoint(x, y)
3. 检查这些关键点返回的元素: 如果所有关键点都返回 body 或 html (即没有业务组件的元素)，说明页面可能白屏
4. 还可以结合检查: document.body.children.length === 0、root 容器 (如 #app) 是否有子元素

判断逻辑:

- 如果 N 个关键点中有超过阈值 (如 80%) 返回的是 body/html，且 root 容器没有子元素，判定为白屏
- 触发白屏告警，上报当前 DOM 快照和错误信息

局限性:

1. 误判: 某些页面本身就是极简设计 (如只有少量文字的页面)，关键点可能确实都落在空白区域
2. 漏判: 页面部分白屏 (如 header 正常但内容区域空白) 可能不会被检测到
3. 采样时机: 如果 SPA 的内容渲染是异步的，采样时内容可能还没加载完

更可靠的方案:

1. 业务主动上报: 在关键业务组件的 useEffect 中调用 sdk.reportReady()，如果超时未收到 ready 信号则判定白屏
2. MutationObserver 监听: 监听 root 容器的 DOM 变化，如果长时间没有子元素插入则判定白屏
3. 截图对比: 使用 html2canvas 或服务端 Puppeteer 截图，与正常页面截图做像素对比

### 3.5 Sentry SDK - 性能指标采集

Web Vitals 指标:

1. LCP (Largest Contentful Paint): 最大内容绘制时间。衡量页面主要内容加载完成的时间。通过 PerformanceObserver 监听 'largest-contentful-paint' 类型获取。LCP 的值是最后一个 LCP 候选元素的时间 (因为更大的元素可能在后面渲染)。

2. FCP (First Contentful Paint): 首次内容绘制时间。衡量从导航开始到页面任何内容 (文本、图片、SVG 等) 首次绘制的时间。通过 PerformanceObserver 监听 'paint' 类型，取 name === 'first-contentful-paint' 的条目。

3. CLS (Cumulative Layout Shift): 累积布局偏移。衡量页面视觉稳定性。通过 PerformanceObserver 监听 'layout-shift' 类型，累加所有 non-input 的 layout shift 的 score。

4. INP (Interaction to Next Paint): 交互到下次绘制延迟。衡量页面对用户交互的响应速度。通过 PerformanceObserver 监听 'event' 类型 (需要 durationThreshold 配置)，取所有交互事件中延迟最大的 P98 值。

Observer API 使用:

- PerformanceObserver: 核心 API，注册监听不同类型的 performance entries
- 注意: 某些类型 (如 layout-shift, event) 需要设置 buffered: true 来获取历史数据

longTask 检测:

- 通过 PerformanceObserver 监听 'longtask' 类型
- 阈值: 浏览器定义为 50ms (任何超过 50ms 的 task 都会被报告)
- longTask 的 attribution 信息可以关联到具体的性能瓶颈: 哪个 frame、哪个 URL 导致的长时间任务
- 结合 longTask 的发生时间和页面操作时间，可以定位到具体的卡顿场景

### 3.6 Sentry SDK - rrweb 故障现场重放

rrweb 录制原理:

1. DOM 快照 (Full Snapshot): 录制开始时，序列化整个 DOM 树为 JSON 结构。每个节点被赋予唯一的 id，记录节点类型、标签名、属性、文本内容等。

2. 增量记录 (Incremental Snapshot): 通过 MutationObserver 监听 DOM 变化，记录每次变化的 diff:
   - 节点新增/删除
   - 属性变化
   - 文本变化
   - 滚动位置变化
...
3. 时间戳: 每个快照和增量记录都带有时间戳，回放时按时间顺序重放

4. 特殊处理:
   - Canvas/WebGL 内容需要特殊序列化
   - iframe 内容需要单独录制
   - CSS 样式表变化需要记录
   - 敏感信息 (如密码输入) 需要脱敏
...
- DOM 序列化 (full snapshot): 一次性开销，大型 DOM (数千节点) 可能耗时几十毫秒
- MutationObserver 增量记录: 持续开销，页面变化频繁时 (如动画) 会产生大量记录数据
- 内存占用: 录制数据在内存中累积，长时间录制可能占用几十 MB

平衡录制精度和性能的策略:

1. 延迟启动: 页面加载稳定后才开始录制
2. 采样率: 不是每个用户都录制，只对一定比例 (如 1%) 的用户开启
3. 限制录制时长: 最多录制最近 N 分钟 (环形缓冲区)
4. 过滤不必要的变化: 忽略动画相关的高频 DOM 变化 (如 requestAnimationFrame 驱动的变化)
5. slimDOM 选项: 跳过 comment 节点、script 标签内容等不影响回放的节点

gzip 压缩比: 文本数据 (JSON) 的 gzip 压缩比通常在 5:1 到 10:1 之间。一段几分钟的录制数据原始大小可能几百 KB 到几 MB，gzip 后通常缩小到几十 KB 到几百 KB。
...
1. navigator.sendBeacon (首选):
   - 特点: 浏览器原生 API，专为数据上报设计。数据被放入浏览器队列，即使页面卸载也能保证发送
   - 适用场景: 页面关闭/跳转时的数据上报 (beforeunload)
   - 优势: 不阻塞页面卸载、可靠性高、API 简单
   - 限制: 只支持 POST、数据大小有限制 (浏览器实现不同，通常 64KB)

2. Image beacon (二级降级):
   - 特点: 创建 Image 对象，将数据编码到 URL query 参数中
   - 适用场景: sendBeacon 不可用时 (部分旧浏览器)
   - 优势: 兼容性极好 (几乎所有浏览器)、不受 CORS 限制
   - 限制: 只能 GET、URL 长度有限制 (2KB 左右)、数据传输量小

3. fetch keepAlive (三级降级):
   - 特点: fetch API 的 keepalive: true 选项，允许请求在页面卸载后继续
   - 适用场景: 需要发送较大数据或需要自定义 header 时
   - 优势: 灵活 (支持 POST、自定义 header)、数据量大
   - 限制: 兼容性不如前两者、keepalive 请求有总数据量限制 (64KB)

离线策略:

- 上报失败时 (网络错误)，将数据序列化后存入 localStorage
- localStorage 容量限制: 通常为 5-10 MB (因浏览器而异)
- 我的实现: 设置 localStorage 的容量上限 (如 1MB)，超出时按 FIFO 策略淘汰旧数据
- 监听 window.addEventListener('online') 事件，网络恢复后自动触发刷盘
- 刷盘策略: 分批次发送 (如每次发送 10 条)，批次之间间隔 1-2 秒，避免瞬间大量请求对服务端造成压力
- 如果刷盘过程中又离线，停止发送，剩余数据保留在 localStorage

### 3.8 Sentry SDK - 包体积优化

SDK 体积: core 模块 gzip 后约 3-4KB，加上常用 plugin (错误捕获 + 性能采集) 约 6-8KB。如果加上 rrweb 录制模块，总体积会增长到 40-50KB gzip (rrweb 本身就是约 35KB)。

包体积优化手段:

1. tree-shaking: 所有模块使用 ESM 导出，未使用的 plugin 不会被打包
2. 按需引入: 用户可以只引入需要的 plugin (如只需要错误捕获，不引入 rrweb)
3. 动态 import: rrweb 录制模块通过 dynamic import 异步加载，不影响 SDK 初始化时的包体积
4. 依赖精简: 不依赖大型第三方库，工具函数自己实现 (如简单的 LRU cache)
5. 代码压缩: 使用 terser 压缩，去除注释、缩短变量名

如果 SDK 体积仍然过大:

1. 异步加载 SDK: 在页面首屏渲染完成后 (requestIdleCallback) 再加载 SDK
2. CDN 加载: SDK 通过 CDN script 标签异步加载，利用浏览器缓存
3. 拆分加载: core 同步加载 (体积小)，plugin 按需异步加载
4. Web Worker: 将数据处理和上报逻辑放到 Worker 中，减少主线程负担
5. 服务端采样: 服务端下发配置，控制客户端是否加载重量级 plugin (如 rrweb)

### 3.9 CLI Coding Agent - 架构设计

swifty-cli 的 5 层架构 (从底到顶):

1. Provider 层 (最底层):
   - 职责: LLM provider 适配层，封装不同 LLM (OpenAI, Anthropic, Google, 本地模型) 的 API 差异
   - 提供统一的接口: chat completion, streaming, tool calling
   - 支持 SSE 流式输出的统一处理
...
2. Tool 层:
   ...
   - Agent: 子 agent 管理
   - 每个工具有明确的输入/输出 schema (用 Zod 定义)

3. Permission 层:
   - 职责: 权限校验和安全控制
   - 拦截所有工具调用，在执行前进行权限检查
   - 支持多种权限模式 (全自动、需确认、只读等)

4. Agent 层:
   - 职责: Agent Loop 和 ReAct 推理循环
   - 管理对话上下文 (消息历史)
   - 驱动 "推理 -> 工具调用 -> 观察结果" 的循环
   - 上下文压缩、记忆管理
...
5. Interface 层 (最顶层):
   ...
   - CLI 的输入/输出处理
   - 会话管理 (创建、恢复、持久化)
   - Slash Command、Skill、Hook 的处理

层间通信: 从上到下是调用关系，从下到上是事件/回调关系。Interface 层接收用户输入 -> Agent 层处理 -> 通过 Permission 层校验 -> Tool 层执行 -> Provider 层调用 LLM。Provider 层的流式输出通过回调/AsyncIterator 向上传递。

分层的好处:

1. 可替换: 换一个 LLM provider 只需修改 Provider 层
2. 可测试: 每层可以独立 mock 和单测
3. 可复用: Tool 层和 Provider 层可以被其他项目复用
4. 关注点分离: 权限、工具、推理逻辑互不耦合

### 3.10 CLI Coding Agent - Agent Loop 与 ReAct

一次完整的推理-行动循环:

1. 用户输入消息，Agent 将用户消息追加到对话历史

2. Agent 调用 Provider 层的 chat completion API，传入:
   - System Prompt (角色定义、工具描述、约束)
   - 对话历史
   - 可用工具列表 (JSON Schema 格式)

3. LLM 返回响应，可能包含:
   - 纯文本 (Thought + 最终回答): 任务完成，直接展示给用户
   - Tool Use (Action): LLM 决定调用工具，包含工具名和参数

4. 如果是 Tool Use:
   a. 解析 LLM 输出的工具调用 (JSON 格式或通过 tool_call 结构化输出)
   b. 通过 Permission 层校验权限
   c. 调用 Tool 层执行工具，获取结果 (Observation)
   d. 将工具调用和结果追加到对话历史
   e. 回到步骤 2，继续下一轮循环
...
LLM 输出解析: 不同 LLM provider 的工具调用格式不同。OpenAI 用 tool_calls 结构化字段; Anthropic 用 tool_use content block。Provider 层统一封装这些差异，向 Agent 层暴露统一的 ToolCall 结构。

异常处理:

- LLM 输出格式错误 (如 JSON 解析失败): 将错误信息作为 Observation 反馈给 LLM，让它重新输出
- 工具调用失败: 将错误信息和堆栈作为 Observation 反馈给 LLM，LLM 可以决定是否重试或换方案
- 超时: 工具执行设置超时 (如 Shell 命令默认 120s)，超时后返回超时错误
- 权限拒绝: 用户拒绝权限时，将 "用户拒绝了此操作" 作为 Observation 反馈

### 3.11 CLI Coding Agent - 权限系统

5 层权限:

1. Allow (白名单): 明确允许的工具和命令模式。如 "allow all Read operations", "allow npm test"
2. Ask (需确认): 默认模式，每次工具调用需要用户确认。显示工具名、参数、影响范围
3. Deny (黑名单): 明确禁止的操作。如 "deny rm -rf", "deny git push --force"
4. Scope (作用域): 限制工具的操作范围。如文件操作限制在项目目录内，Shell 命令限制在只读命令
5. Mode (全局模式): 覆盖整体行为。如 bypassPermissions (全部允许)、plan (只规划不执行)、readOnly (只读)

权限定义:

- 通过配置文件 (settings.json) 定义，支持多层级 (全局、项目级、会话级)
- 权限规则支持通配符匹配 (如 "allow npm \*" 允许所有 npm 命令)
- 支持复合规则 (如 "allow bash if command matches pattern")

权限校验流程:

1. 工具调用进入 Permission 层
2. 检查 Deny 列表: 匹配则拒绝
3. 检查 Allow 列表: 匹配则放行
4. 检查 Scope 限制: 如文件路径是否在允许范围内
5. 如果以上都没有匹配，根据 Mode 决定: Ask 模式下提示用户确认，bypassPermissions 模式下直接放行

不同能力的权限粒度:

- 文件操作: 按路径 + 操作类型 (read/write/delete) 控制
- Shell 命令: 按命令模式 (正则/通配符) 控制
- 网络请求: 按域名 + HTTP 方法控制
- Agent 创建: 按 agent 类型 + 模式控制

运行时授权:

- Ask 模式下，每次工具调用弹出确认提示
- 用户可以选择: 允许本次、允许同类操作 (记住选择)、拒绝、拒绝并终止
- 用户的授权选择可以持久化到配置文件，下次无需再确认

### 3.12 CLI Coding Agent - 上下文压缩

上下文压缩的必要性:

- LLM 的上下文窗口有限 (如 128K tokens)，长对话 + 大量工具调用结果会快速耗尽
- 即使不超出窗口，上下文越长，LLM 的推理质量越差 (注意力分散、关键信息被稀释)
- Token 越多，API 调用成本越高

压缩算法 (混合策略):

1. 工具结果压缩 (最先触发):
   - 早期的工具调用结果 (如 10 轮之前的) 用 LLM 生成摘要替代原始输出
   - 例如: 一次 grep 返回了 200 行匹配结果，压缩为 "在 X 文件中找到 15 处匹配，关键匹配在 Y 文件的 Z 行"
   - 文件读取结果压缩为 "文件 X 共 N 行，关键内容在第 M-K 行，包含..."

2. 对话历史压缩 (中等触发):
   - 早期的 Thought 和推理过程被压缩为简短的决策摘要
   - 保留关键决策点和最终结论，去除中间推理步骤

3. 滑动窗口 (最后手段):
   - 当 token 数仍然超标时，丢弃最早的对话轮次
   - 保留 System Prompt + 最近 N 轮 + 压缩后的历史摘要

保证关键信息不丢失:

- 用户最近的操作和对话始终保留原始内容
- 文件路径、错误信息、关键代码片段等结构化信息提取后保留
- 压缩由 LLM 执行，Prompt 中明确要求保留: 文件路径、行号、错误信息、关键决策

触发条件:

- 每次工具调用返回结果后检查总 token 数
- 当 token 数达到上下文窗口的 70% 时，触发工具结果压缩
- 当 token 数达到 85% 时，触发对话历史压缩
- 当 token 数达到 95% 时，触发滑动窗口

### 3.13 CLI Coding Agent - 记忆系统

自动记忆提取:

提取时机: 在每次会话结束前 (或会话空闲时)，Agent 会审查整个对话历史，提取值得跨会话保留的信息。

提取内容:

1. 用户偏好: 如 "用户偏好 TypeScript 严格模式"、"用户不喜欢代码中的注释"
2. 项目知识: 如 "项目使用 pnpm 作为包管理器"、"测试命令是 pnpm test"
3. 错误经验: 如 "项目 X 的构建需要先执行 Y 步骤"
4. 工作流模式: 如 "用户习惯先写测试再写实现"

提取方式: 用 LLM 分析对话，System Prompt 要求它识别上述类型的信息，输出结构化的记忆条目 (类型、内容、来源)。

定期记忆整理 (consolidation):

- 当记忆条目数量超过阈值 (如 50 条) 时触发
- LLM 审查所有记忆: 合并重复的、删除过时的、修正不准确的
- 按主题分组整理 (用户偏好、项目知识、错误经验等)

记忆如何影响后续会话:

- 会话启动时，根据当前上下文 (项目路径、用户问题) 检索相关记忆
- 相关记忆注入到 System Prompt 中，影响 Agent 的行为
- 例如: 记忆中 "项目使用 pnpm"，Agent 在后续会话中会自动使用 pnpm 而不是 npm

避免记忆膨胀:

- 每条记忆有 TTL (过期时间)，长期未被匹配的记忆自动降低优先级
- 记忆整理时合并重复条目
- 总记忆条数限制上限
- 记忆按相关度排序，低相关度的记忆不注入 System Prompt

### 3.14 CLI Coding Agent - Subagent 与 Agent Team

Subagent 和主 Agent 的通信机制:

- 主 Agent 通过 Agent 工具创建 Subagent，传入任务描述和上下文
- Subagent 运行在独立的上下文中 (有自己的消息历史)，但共享文件系统 (除非使用 worktree 隔离)
- Subagent 完成后，将结果返回给主 Agent (作为工具调用的返回值)
- 主 Agent 可以查看 Subagent 的输出，决定是否继续、修改指令、或启动新的 Subagent

预定义 vs Fork Subagent:

- 预定义 Subagent: 在配置中定义的具有特定角色和工具限制的 agent (如 "explore" agent 只有读权限，"plan" agent 只做规划)
- Fork Subagent: 从主 Agent fork 出来的副本，继承主 Agent 的上下文和工具权限，用于并行执行子任务

worktree 文件隔离:

- 解决的问题: 多个 agent 同时修改同一个文件时的冲突问题
- 实现: 使用 git worktree 为每个 Subagent 创建独立的工作目录 (分支)
- Subagent 在自己的 worktree 中修改文件，不影响主 Agent 的工作目录
- 完成后，主 Agent 可以 merge Subagent 的变更

Agent Team 的任务分配和协调:

1. Team Lead 创建团队 (TeamCreate)，建立共享的任务列表
2. 使用 TaskCreate 创建任务，定义每个任务的描述和依赖关系
3. 通过 Agent 工具 spawn teammate，每个 teammate 加入团队
4. Teammate 通过 TaskUpdate 领取任务 (设置 owner)、标记进度、标记完成
5. Teammate 之间通过 SendMessage 通信
6. 任务完成后，Team Lead 通过 shutdown_request 通知 teammate 退出

### 3.15 CLI Coding Agent - MCP 接入

MCP 协议规范:

- MCP 定义了 AI Agent 与外部工具/数据源之间的标准协议
- 传输层: 支持 stdio (标准输入/输出) 和 SSE (Server-Sent Events) 两种传输方式
- 消息格式: 基于 JSON-RPC 2.0，包含 request、response、notification 三种消息类型
- 核心能力:
  - Tools: 工具调用 (类似函数调用)
  - Resources: 数据资源访问 (类似文件系统)
  - Prompts: 预定义的提示模板

MCP Client 实现:

1. 连接建立: 根据配置启动 MCP server 进程 (stdio) 或连接 SSE endpoint
2. 能力协商: client 和 server 交换各自支持的能力 (initialize 请求)
3. 工具发现: client 调用 tools/list 获取 server 提供的工具列表 (名称、描述、参数 schema)
4. 工具注册: 将 MCP 工具转换为 Agent 可用的工具定义，注入到工具列表中

工具调用流程:

1. Agent 决定调用某个 MCP 工具 (和普通工具没有区别)
2. Agent 层将工具调用转换为 MCP 的 tools/call JSON-RPC 请求
3. 通过传输层发送给 MCP server
4. MCP server 执行工具逻辑，返回结果
5. Client 接收结果，转换为 Agent 层的 Observation 格式

MCP vs 直接定义工具函数:

优势:

1. 生态复用: 社区有大量 MCP server 实现 (GitHub, Slack, Database 等)，接入即可获得新能力
2. 动态扩展: 不需要修改 Agent 代码，只需要配置新的 MCP server 就能增加工具
3. 进程隔离: MCP server 运行在独立进程中，崩溃不影响 Agent
4. 语言无关: MCP server 可以用任何语言实现，不局限于 Agent 的语言
5. 权限隔离: MCP server 可以有自己的权限控制和认证机制

劣势:

1. IPC 开销: 进程间通信有序列化/反序列化开销
2. 调试复杂度: 跨进程调试比进程内调试复杂
3. 依赖管理: 需要管理 MCP server 的安装和运行环境
