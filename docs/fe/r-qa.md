...
3. 空闲回收: 超过一定时间 (如 30s) 没有使用的连接自动关闭，释放资源
...
- 二进制数据通过 Buffer 传递，单次 IPC 开销约 1ms，相比计算耗时 (加密 1MB 数据约 5ms，解析大表文件几十 ms) 占比很小

为什么用进程池而不是在主进程内直接调用:

1. 事件循环阻塞: 加解密和表文件解析是 CPU 密集操作，在主进程内同步调用会阻塞事件循环，高负载下严重影响 Node.js 的并发能力。进程池将计算卸载到独立进程，主进程只做 IO 和调度。
2. 崩溃隔离: .so 内部 segfault 会拖垮所在进程。放在子进程中执行，崩溃只影响单个 worker，主进程监听 exit 事件自动重启补充，服务不中断。
3. 并行计算: 多个子进程可以并行处理加解密请求，吞吐量提升约 3.5 倍。

进程池方案的风险与应对:

1. IPC 序列化开销: 大 Buffer 传输存在拷贝成本，对高频小任务做批量合并，减少 IPC 次数。
2. 内存管理: N-API 绑定层必须保证异常路径也释放 C++ 侧内存。我们用 valgrind 排查出异常路径漏 free 的泄漏，最终用 RAII 智能指针修复。
3. 类型安全: 在绑定层之上封装 TypeScript 类型层，对外提供类型安全的调用接口。
