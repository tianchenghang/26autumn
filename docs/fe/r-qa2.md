...
13. A2UI (Agent to UI) React 框架接入是怎么做的？LLM 输出如何映射到 React 组件？组件渲染的流式更新怎么处理的？
...
Webpack 下的 Module Federation 是 webpack 5 的原生特性，通过 plugins 配置 shared dependencies、exposes、remotes 即可，webpack 的 runtime 会自动处理远程模块的加载、依赖共享和版本协商。Vite 下的接入要复杂得多，因为 Vite 的开发模式基于原生 ESM（不做 bundle），而 Module Federation 的核心机制（remote entry、chunk sharing）依赖打包产物的 runtime 逻辑。@module-federation/vite 的方案是在 Vite 的 plugin 体系中注入自定义的模块解析和转换逻辑，在开发模式下通过 Vite 的 middleware 拦截远程模块请求并代理到远端 dev server，在生产模式下通过 Rollup plugin 在构建时生成 federation runtime。我贡献的主要是修复了开发模式下远程模块热更新不生效的问题：当远程模块的代码变更时，consumer 端对应的虚拟模块没有被 invalidate，导致 HMR 不生效，修复方式是在 Vite 插件的 handleHotUpdate 钩子中手动触发远程 module map 的刷新，并向依赖图注入 invalidate 信号。此外还优化了 shared 依赖的预构建逻辑（将多个 shared 依赖合并为一次 optimizeDeps 调用，预构建时间从约 12 秒降到 3 秒），并补充了 runtime plugin 机制，对齐 Webpack 版 Module Federation 的 runtime plugin API。远程模块加载失败的降级方案是：每个远程模块都配置一个 fallback 组件（通常是一个静态的占位 UI 或功能降级版本），通过 React 的 ErrorBoundary 包裹远程模块的加载点，捕获加载异常后渲染 fallback。同时在监控平台埋点上报加载失败事件，便于排查网络和部署问题。
