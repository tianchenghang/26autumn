...
- [一、项目经历深挖](#一项目经历深挖)
  - [Q1: Sentry SDK 上报 JSError 后如何通过 source map 还原故障现场？](#q1-你项目经历做的-sentry-sdk-上报-jserror-后如何通过-source-map-还原故障现场)
  - [Q2: Sentry SDK 包体积太大影响首屏加载怎么办？](#q2-你项目经历做的-sentry-sdk-包体积太大影响首屏加载怎么办)
  - [Q3: Sentry SDK 用的 rrweb 原理是什么，对性能影响？](#q3-你项目经历做的-sentry-sdk-用的-rrweb-原理是什么对性能影响)
  - [Q4: Sentry SDK 白屏检测的关键点采样算法是怎么实现的？](#q4-你项目经历做的-sentry-sdk-白屏检测的关键点采样算法是怎么实现的)
  - [Q5: Sentry SDK 的 3 级降级上报策略是怎么设计的？](#q5-sentry-sdk-的-3-级降级上报策略是怎么设计的为什么这样设计)
  - [Q6: Sentry SDK 的 LRU 缓存和错误去重是怎么实现的？](#q6-sentry-sdk-的-lru-缓存和错误去重是怎么实现的)
  - [Q7: Sentry SDK 的 Web Vitals 指标是怎么计算的？](#q7-sentry-sdk-的-web-vitals-指标lcpfcpclsinp是怎么计算的)
  - [Q8: CLI Coding Agent 的 ReAct 范式和 Agent Loop 是怎么实现的？](#q8-cli-coding-agent-的-react-范式和-agent-loop-是怎么实现的)
  - [Q9: CLI Agent 的 5 层权限系统是怎么设计的？](#q9-cli-agent-的-5-层权限系统是怎么设计的)
  - [Q10: CLI Agent 的 MCP 协议接入是怎么做的？](#q10-cli-agent-的-mcp-协议接入是怎么做的)
  - [Q11: CLI Agent 的上下文压缩算法是怎么实现的？](#q11-cli-agent-的上下文压缩算法是怎么实现的)
  - [Q12: CLI Agent 的记忆系统是怎么设计的？](#q12-cli-agent-的记忆系统自动提取定期整理是怎么设计的)
  - [Q13: CLI Agent 的 worktree 文件隔离和多 teammate 协作是怎么实现的？](#q13-cli-agent-的-worktree-文件隔离和多-teammate-协作是怎么实现的)
- [二、工作经历深挖](#二工作经历深挖)
...
### Q1: 你项目经历做的 Sentry SDK 上报 JSError 后如何通过 source map 还原故障现场？

A: Source Map 还原是错误追踪系统的核心能力。在 swifty-sentry 中，我实现了一套完整的 Source Map 上传和解析流程。首先在构建阶段，通过 Webpack/Vite 插件在打包完成后自动将 `.map` 文件上传到 SDK 服务端，同时保留生产环境代码的 hash 标识。当 JSError 上报时，SDK 会捕获完整的错误栈信息，包括 `filename`、`lineno`、`colno` 等关键字段。

```javascript
// 错误捕获时提取栈信息
window.addEventListener("error", (event) => {
  const stackFrames = StackTrace.parse(event.error);
  const payload = {
...
服务端收到错误后，会根据 `filename` 中的 hash 值查找对应的 Source Map 文件。这里使用 Mozilla 的 `source-map` 库进行反向映射：通过 `SourceMapConsumer` 加载 `.map` 文件，调用 `originalPositionFor` 方法将压缩后的行列号还原为源码位置。同时还会提取源码上下文（前后各 5 行），生成可视化的错误堆栈视图。

关键优化点有三个：第一，Source Map 文件按 chunk hash 索引，避免版本混乱；第二，服务端缓存已解析的 Source Map，减少重复 IO；第三，对超大 Source Map 文件（>10MB）采用流式解析，避免内存溢出。这套方案在实际项目中将错误定位时间从平均 15 分钟缩短到 30 秒内，准确率 98%+。

权衡方面，Source Map 上传会增加构建时间（约 5-10 秒），存储成本也需要考虑（平均每个版本 50-200MB）。我的做法是在 CI/CD 流程中异步上传，并设置 30 天自动清理策略。

---

### Q2: 你项目经历做的 Sentry SDK 包体积太大影响首屏加载怎么办？

A: SDK 包体积优化是前端监控系统的经典问题。swifty-sentry 初始版本打包后达到 85KB（gzip 后 28KB），对于首屏加载是不可接受的。我通过以下几个维度进行了深度优化。

首先是按需加载架构。将 SDK 拆分为 core（核心上报能力，8KB）和多个 plugin（错误捕获、性能监控、屏幕录制等），用户可以根据需求动态加载：

```javascript
import { init } from "@swifty-sentry/core";
import { ErrorPlugin } from "@swifty-sentry/plugin-error";
import { PerformancePlugin } from "@swifty-sentry/plugin-performance";
...
其次是Tree Shaking 优化。重构代码结构，确保所有导出都是纯函数，避免副作用。使用 `sideEffects: false` 标记 package.json，配合 Rollup 的 `treeshake.moduleSideEffects` 配置，将未使用的功能彻底剔除。同时移除 moment.js 等大依赖，改用 dayjs（2KB）。

第三是延迟加载策略。将 rrweb 屏幕录制模块改为动态 import，仅在用户触发错误时才加载：

```javascript
async function startRecording() {
  const { record } = await import("rrweb");
  return record({ emit: handleEvent });
}
```

最终优化效果：core 包 8KB，完整 SDK 从 85KB 降至 42KB（gzip 14KB），首屏加载时间减少 60ms。进一步优化方向是 Web Worker 上报和 WASM 压缩算法，但考虑到兼容性暂未实施。

---

### Q3: 你项目经历做的 Sentry SDK 用的 rrweb 原理是什么，对性能影响？

A: rrweb（record and replay the web）是一个开源的会话回放库，核心原理是DOM 快照 + 增量事件流。初始化时，rrweb 会遍历整个 DOM 树，将每个节点序列化为 JSON 结构（包含 tagName、attributes、textContent 等），生成初始快照。之后通过 MutationObserver 监听 DOM 变化，记录每次变更的增量事件（节点增删、属性修改、文本变化等）。

```javascript
// rrweb 内部简化逻辑
const snapshot = serializeDocument(document); // 初始快照
const observer = new MutationObserver((mutations) => {
  mutations.forEach((mutation) => {
    emit({
      type: "mutation",
      data: serializeMutation(mutation),
    });
  });
});
...
对性能的影响主要体现在三个方面：CPU 开销（DOM 序列化和 MutationObserver 回调）、内存占用（事件队列缓存）、网络传输（事件数据上报）。在我的实测中，开启 rrweb 后页面 FCP 增加 80-150ms，内存占用增加 15-30MB，CPU 使用率峰值增加 8-12%。

为了降低性能影响，我做了几个关键优化：第一，采样策略，仅在用户触发错误前 30 秒开始录制，而非全程录制；第二，数据压缩，使用 gzip 将事件数据压缩 70-80%；第三，分片上报，每 10 秒或累积 100KB 时上报一次，避免单次请求过大；第四，Web Worker 处理，将序列化和压缩逻辑放到 Worker 线程，避免阻塞主线程。

优化后的效果：性能影响降低到 FCP +30ms、内存 +8MB、CPU +3%，基本可以接受。对于移动端或低端设备，还提供了关闭录制的降级方案。

---

### Q4: 你项目经历做的 Sentry SDK 白屏检测的关键点采样算法是怎么实现的？

A: 白屏检测是前端监控的重要能力，传统方案（如检测 `document.body.children.length === 0`）误报率高。我设计了一套关键点采样 + 多维度判定的算法，准确率提升到 95%+。

核心思路是在页面关键区域（如 header、main content、footer）埋入采样点，通过检测这些点的可见性判断是否白屏：

```javascript
const samplingPoints = [
  { selector: "#app > header", weight: 0.3 },
  { selector: "#app > main", weight: 0.5 },
  { selector: "#app > footer", weight: 0.2 },
];
...
为了应对 SPA 动态渲染，检测时机选在 `load` 事件后 3 秒和 5 秒各执行一次（给异步数据请求留足时间）。如果两次都判定为白屏，才触发上报，避免误报。同时记录页面 HTML 快照和 console 日志，辅助定位问题。

权衡方面，采样点配置需要业务方配合，增加了接入成本。为此我提供了自动推断模式：通过启发式算法分析 DOM 结构，自动识别 header/main/footer 区域。虽然准确率略低（85%），但降低了接入门槛。

---

### Q5: Sentry SDK 的 3 级降级上报策略是怎么设计的？为什么这样设计？

A: 3 级降级上报策略是为了在不同网络环境和浏览器兼容性下保证错误数据的可靠上报。设计遵循渐进式降级原则，优先使用最优方案，失败时自动回退。

第一级：sendBeacon API。这是现代浏览器提供的专用数据上报接口，优势在于异步、不阻塞页面卸载、可靠性高：
...
第二级：Image Beacon。当 sendBeacon 不可用（如 IE11）或失败时，降级为 1x1 像素图片请求。通过 URL 参数传递数据（GET 请求，最大 2KB），兼容性好但容量受限：

```javascript
function reportWithImage(data) {
  const img = new Image();
  const query = encodeURIComponent(JSON.stringify(data).slice(0, 2000));
  img.src = `/api/report?data=${query}`;
}
```

第三级：fetch keepAlive。当数据量超过 2KB 且前两级都失败时，使用 fetch 的 `keepalive: true` 选项。这允许请求在页面卸载后继续执行，但兼容性较差（Safari 不支持）：
...
设计原因：sendBeacon 是最佳实践（W3C 标准），但兼容性不完整；Image Beacon 兼容性最好但容量受限；fetch keepAlive 是兜底方案。三级策略覆盖了 99% 的场景，确保数据不丢失。实际监控数据显示，sendBeacon 成功率 92%，Image Beacon 兜底 6%，fetch keepAlive 兜底 1.5%，整体上报成功率 99.5%。

---

### Q6: Sentry SDK 的 LRU 缓存和错误去重是怎么实现的？

A: LRU（Least Recently Used）缓存在 SDK 中用于两个场景：错误去重（避免同一错误重复上报）和离线缓存（网络异常时暂存数据）。

错误去重的 LRU 实现基于 HashMap + 双向链表，时间复杂度 O(1)：
...
// 错误去重使用
const errorCache = new LRUCache(100);

function shouldReport(error) {
  const key = generateErrorKey(error); // hash(message + stack)
  if (errorCache.get(key)) {
    return false; // 已上报过
  }
  errorCache.put(key, Date.now());
  return true;
}
```

去重 key 的生成策略：对 `error.message` + `error.stack` 前 200 字符做 SHA-256 hash，取前 16 位。这样即使同一错误在不同时间触发，也能被识别并去重。缓存容量设为 100，足以覆盖单次会话的错误类型，同时控制内存占用（< 10KB）。

离线缓存则使用 localStorage，容量更大（5MB）。当 `navigator.onLine === false` 时，错误数据序列化为 JSON 数组存入 `sentry_offline_queue`。监听 `online` 事件，网络恢复后批量上报并清空队列。为避免一次性请求过多，采用分批上报（每批 10 条）和指数退避重试。

---

### Q7: Sentry SDK 的 Web Vitals 指标（LCP/FCP/CLS/INP）是怎么计算的？

A: Web Vitals 是 Google 提出的核心性能指标，我使用 `web-vitals` 库（Google 官方）采集，但做了自定义处理和上报逻辑。

FCP（First Contentful Paint）：首次内容绘制时间，标记浏览器渲染第一个 DOM 内容（文本、图片、SVG 等）的时间点。通过 PerformanceObserver 监听 `paint` 类型获取：
...
LCP（Largest Contentful Paint）：最大内容绘制时间，标记视口内最大元素完成渲染的时间。通过监听 `largest-contentful-paint` 类型，取最后一次回调的值（因为 LCP 可能在页面加载过程中多次更新）：
...
CLS（Cumulative Layout Shift）：累积布局偏移，衡量页面视觉稳定性。监听 `layout-shift` 事件，累加所有 `hadRecentInput === false` 的偏移值：
...
INP（Interaction to Next Paint）：交互到下次绘制延迟，衡量页面响应性。监听 `event` 类型，记录用户交互（点击、键盘）到下次渲染的时间差。取所有交互的 P98 值作为最终 INP。

上报时机选在 `visibilitychange` 的 `hidden` 状态，确保指标完整。同时附带页面 URL、用户 ID、设备信息等上下文数据，便于后续分析。

---

### Q8: CLI Coding Agent 的 ReAct 范式和 Agent Loop 是怎么实现的？

A: ReAct（Reasoning + Acting）是一种结合推理和行动的 Agent 范式，核心思想是让 LLM 在每一步先进行思考（Thought），再决定行动（Action），观察结果（Observation）后继续推理。swifty-cli 的 Agent Loop 基于此实现。
...
async function agentLoop(task: string) {
  ...
  while (true) {
    ...
    const action = extractAction(response); // { tool: 'readFile', params: {...} }
...
System Prompt 的设计至关重要，需要明确指导 LLM 的输出格式：

```
You are a helpful coding assistant. Use the following format:
...
工具执行层采用注册表模式，每个工具定义输入 schema 和执行逻辑：
...
关键优化：第一，流式输出，使用 SSE 实时显示 Thought 过程，提升用户体验；第二，工具调用权限，危险操作（如写文件、执行命令）需要用户确认；第三，上下文压缩，当历史过长时自动摘要早期对话，保留关键信息。

---

### Q9: CLI Agent 的 5 层权限系统是怎么设计的？

A: 权限系统是 Agent 安全的核心，swifty-cli 设计了 5 层递进式权限模型，平衡安全性和用户体验。

Layer 1: 只读操作（自动允许）。包括读取文件、搜索代码、查询文档等无副作用操作。这些操作不会修改任何状态，可以自动执行，无需用户干预。

Layer 2: 受控写操作（首次询问，后续自动）。包括编辑文件、创建目录等。首次执行时弹出确认提示，用户可选择"本次允许"或"本会话始终允许"。选择后者会将该操作加入会话白名单。

```typescript
async function requestPermission(operation: Operation): Promise<boolean> {
  if (operation.level === 1) return true;

  if (sessionWhitelist.has(operation.signature)) {
    return true;
  }

  const answer = await prompt(`Allow ${operation.description}? [y/n/always]`);
  if (answer === "always") {
    sessionWhitelist.add(operation.signature);
    return true;
  }
  return answer === "y";
}
...
Layer 3: 危险操作（每次确认）。包括删除文件、执行 shell 命令、git push 等。每次执行都需要用户明确确认，不允许自动放行。命令执行前还会显示完整的命令内容，供用户审查。

Layer 4: 系统级操作（二次确认 + 警告）。包括修改系统配置、安装全局包、访问敏感目录等。除了确认提示，还会显示风险警告，并要求用户输入操作名称二次确认（如输入 "delete" 确认删除）。

Layer 5: 禁止操作（直接拒绝）。包括访问 `/etc`、`/root` 等系统目录、执行 `rm -rf /` 等危险命令、修改 Agent 自身配置等。这些操作会被直接拒绝，并记录到审计日志。

权限配置支持项目级 `.swifty/permissions.json`，团队可以统一权限策略。同时提供 `--yolo` 模式跳过所有确认（仅限本地开发环境）。

---

### Q10: CLI Agent 的 MCP 协议接入是怎么做的？

A: MCP（Model Context Protocol）是 Anthropic 提出的标准化协议，用于连接 LLM 与外部工具和数据源。swifty-cli 实现了完整的 MCP Client，支持接入任意 MCP Server。

MCP 协议基于 JSON-RPC 2.0，定义了三个核心能力：Tools（工具调用）、Resources（数据读取）、Prompts（提示模板）。swifty-cli 主要使用 Tools 能力。
...
class MCPClient {
  ...
  async connect(serverConfig: MCPServerConfig) {
    ...
    const capabilities = await this.call("initialize", {
      clientInfo: { name: "swifty-cli", version: "1.0.0" },
    });
...
配置文件 `.swifty/mcp.json` 支持声明多个 MCP Server：
...
Agent 启动时会加载所有 MCP Server 的 Tools，合并到内置工具列表中。调用时通过 `tool_name` 前缀区分来源（如 `mcp_github_create_issue`）。关键挑战是工具描述的标准化：不同 MCP Server 的描述格式不一，我做了统一转换，确保 LLM 能正确理解工具用途。

---

### Q11: CLI Agent 的上下文压缩算法是怎么实现的？

A: LLM 的上下文窗口有限（如 Claude 200K、GPT-4 128K），长对话会超出限制。swifty-cli 实现了滑动窗口 + 摘要压缩的混合策略。

策略 1: 滑动窗口。保留最近 N 轮对话（默认 10 轮），早期对话直接丢弃。适用于简单任务，但会丢失关键上下文。

策略 2: 摘要压缩。当上下文接近 80% 容量时，触发压缩：

```typescript
async function compressContext(history: Message[]): Promise<Message[]> {
...
策略 3: 分层压缩。对不同类型的内容采用不同压缩率：

- 工具调用结果：保留关键输出，丢弃冗余日志（如文件内容只保留修改部分）
- Thought 过程：压缩为 1-2 句话
- 代码片段：只保留 diff，不保留完整文件

```typescript
function compressToolResult(result: string, toolName: string): string {
  if (toolName === "readFile" && result.length > 2000) {
    // 文件内容只保留前 1000 + 后 500 字符
    return (
      result.slice(0, 1000) + "\n... [truncated] ...\n" + result.slice(-500)
    );
  }
...
压缩触发时机：每次 LLM 调用前检查 token 数，超过阈值（如 150K）时触发。压缩后 token 数通常降至 50-80K，为后续对话留出空间。

---

### Q12: CLI Agent 的记忆系统（自动提取+定期整理）是怎么设计的？

A: 记忆系统分为短期记忆（会话内）和长期记忆（跨会话），目标是让 Agent "记住"用户的偏好和项目特征。

短期记忆：当前会话的完整对话历史，存储在内存中。会话结束时序列化为 JSON 文件，保存到 `.swifty/sessions/{session_id}.json`，支持会话恢复。

长期记忆：通过 LLM 自动提取关键信息，存储到 `.swifty/memory.json`：

```json
{
  "user_preferences": {
    "coding_style": "prefer functional components",
  ...
  "project_knowledge": {
    ...
    "key_directories": ["src/components", "src/hooks"]
  },
  "learned_patterns": [
    "User often forgets to run tests before committing",
    "User prefers inline comments over JSDoc"
  ]
}
```

提取时机：每次会话结束时，使用 LLM 分析对话，提取有价值的信息：

```typescript
async function extractMemory(sessionHistory: Message[]): Promise<MemoryUpdate> {
  const prompt = `Analyze this conversation and extract:
1. User preferences (coding style, conventions, language)
2. Project knowledge (frameworks, tools, structure)
3. Learned patterns (recurring issues, habits)

Return as JSON. Only include new or updated information.

Conversation:
${formatMessages(sessionHistory)}`;
...
定期整理：每 10 次会话后，触发记忆整理。使用 LLM 合并重复项、删除过时信息、优化结构：

```typescript
async function consolidateMemory(memory: Memory): Promise<Memory> {
  const prompt = `Consolidate this memory data:
1. Merge duplicate or conflicting entries
2. Remove outdated information
3. Organize into clear categories
4. Prioritize most recent preferences
...
使用记忆时，Agent 会在 System Prompt 中注入相关记忆片段，如 "User prefers TypeScript strict mode" 或 "This project uses Vite for building"。这样 LLM 能给出更贴合用户习惯的建议。

---

### Q13: CLI Agent 的 worktree 文件隔离和多 teammate 协作是怎么实现的？

A: Worktree 隔离和多 Agent 协作是 swifty-cli 的高级特性，灵感来自 Git worktree 和人类团队协作模式。

Worktree 隔离：每个 Subagent 在独立的工作目录中执行任务，避免文件冲突。实现基于 Git worktree：

```bash
# 主 Agent 创建 worktree
git worktree add .swifty/worktrees/agent-1 -b feature/agent-1-task

# Subagent 在隔离目录中工作
cd .swifty/worktrees/agent-1
# 执行文件修改、测试等操作
```

每个 worktree 是完整的 Git 仓库副本，Subagent 可以自由修改文件、运行测试，互不干扰。任务完成后，主 Agent 合并各个 worktree 的改动：

```typescript
async function mergeWorktrees(agentBranches: string[]) {
  for (const branch of agentBranches) {
    await exec(`git merge ${branch} --no-ff -m "Merge ${branch}"`);
  }
  // 清理 worktree
...
多 Agent 协作：通过 Agent Team 实现任务分解和并行执行：

```typescript
class AgentTeam {
  private agents: Map<string, Subagent> = new Map();
  private mailbox: Map<string, Message[]> = new Map();

  async spawn(name: string, task: string) {
    const agent = new Subagent({
      name,
      task,
      worktree: `.swifty/worktrees/${name}`,
  ...
  async sendMessage(from: string, to: string, message: string) {
    const queue = this.mailbox.get(to) || [];
    queue.push({ from, content: message, timestamp: Date.now() });
    this.mailbox.set(to, queue);
  }

  async coordinate() {
    // 主 Agent 作为协调者，分配任务、汇总结果
    while (!this.isTaskComplete()) {
      const updates = await this.collectUpdates();
      const nextActions = await this.planNextActions(updates);
      await this.dispatchActions(nextActions);
    }
  }
}
```

通信机制：Agent 之间通过 mailbox 异步通信，支持发送文本消息和文件引用。主 Agent 负责协调，避免冲突（如两个 Agent 修改同一文件）。

实际场景：用户要求"重构认证模块并补充测试"，主 Agent 拆分为两个子任务：Agent-1 负责重构代码，Agent-2 负责编写测试。两者并行执行，完成后主 Agent 合并结果并运行集成测试。
