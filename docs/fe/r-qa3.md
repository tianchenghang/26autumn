...
A: TikTok Performance 团队负责收集和分析各端上报的终端设备性能数据，日均处理约 2 亿条事件。我参与设计了基于 Kafka + ClickHouse 的实时数据管道。
...
Kitex Consumer：后端基于 CloudWeGo Kitex 框架，使用 Thrift IDL 定义服务接口。Consumer 从 Kafka 拉取消息后，进行数据清洗（字段校验、时间戳转换、IP 解析）和聚合（5 分钟窗口内的 P50/P90/P99 延迟），然后批量写入 ClickHouse。
...
A: 多层缓存是为了平衡性能和数据一致性，TikTok Performance 平台使用了 L1（BFF 进程内 LRU）+ L2（Redis）+ L3（ClickHouse / MySQL）的三层架构；Go 后端服务内部另有进程内热点缓存 + singleflight 做热点保护。

Go 服务端进程内热点缓存（Go map + LRU）。容量 1000 条，TTL 5 分钟。适用于热点数据（如当前活跃的设备维度配置）。使用 `sync.RWMutex` 保证并发安全：
...
L3: MySQL / ClickHouse。持久化存储，作为最终数据源。配置和元数据查 MySQL，明细和聚合分析查 ClickHouse。当 L1/L2 都 miss 时，穿透到存储层并回填缓存。
...
实际效果：BFF L1 缓存在热门面板上的命中率约 60%，Redis L2 全局命中率约 45%，前端面板查询 P99 延迟控制在 300ms 以内。
...
3. 稳定 key：key 使用数据的唯一 ID 而非数组索引，保证滚动复用时组件状态不错乱，配合 React.memo 减少重渲染
...
性能优势：Kitex 使用自研的 Netpoll 网络库（基于 epoll），相比标准库 net 性能提升 30%+。Thrift 二进制协议比 JSON 序列化更快，体积小 30%~50%。实测单机 QPS 稳定在 5 万+，P99 延迟 < 5ms。

---

### Q18: 腾讯 IEG 的 React 类组件迁移到函数组件是怎么做的？遇到了哪些坑？

A: 腾讯 IEG 的 NoSQL 数据库管理系统是一个有多年历史的 React 项目，包含 80+ 个组件，其中 60% 以上是类组件。我负责核心模块（数据浏览、查询构建器）的迁移工作。
...
迁移效果：代码量减少约 25%，单元测试覆盖率显著提升，与状态相关的 bug 数量明显下降，组件渲染性能提升 10-20%（避免了类组件的 this 绑定和继承开销）。
...
A: 腾讯 IEG 的 NoSQL 管理系统后端（Node.js）需要与底层存储引擎通过 TCP 长连接通信。为避免频繁创建/销毁连接的开销，我用 TypeScript 实现了一个通用的 TCP 连接池。
...
interface PoolStats {
  openCount: number; // 当前打开的连接数
  idleCount: number; // 当前空闲连接数
  waitCount: number; // 等待获取连接的请求数
  hitCount: number; // 命中空闲连接的次数
  missCount: number; // 未命中新建连接的次数
}
...
class ConnectionPool {
  ...
  constructor(private readonly options: PoolOptions) {
    for (let i = 0; i < options.minIdle; i++) {
...
健康检查：后台定时器定期检测空闲连接的有效性（发送心跳包），剔除失效连接。同时维护最小空闲连接数，避免突发流量导致连接不足。

配置参数：

- minIdle: 4（根据平均 QPS 估算，启动时预热创建）
- maxOpen: 20（根据后端承载能力设置）
- maxLifetime: 30 分钟（避免长时间持有连接导致后端状态不一致）
- idleTimeout: 30 秒（及时释放空闲连接，避免资源浪费）
- healthCheckInterval: 10 秒
...
A: 腾讯 IEG 的 NoSQL 管理系统需要调用公司内部 C++ 库进行数据加解密和表文件解析，这些模块以 `.so` 动态链接库形式提供。我们采用 child_process 进程池 + 子进程内 N-API 绑定的架构调用这些库。

技术选型：对比了三种方案。N-API addon 直接在主进程调用性能最好，但 CPU 密集计算会阻塞事件循环，且 .so 崩溃会拖垮主进程；FFI 类方案 (ffi-napi) 免编译但调用开销大、稳定性差；最终选择在子进程内通过轻量 N-API 绑定层加载 .so，主进程通过 IPC 分发任务，兼顾性能、隔离性和事件循环不阻塞。

架构示意：

![加解密进程池架构](../assets/crypto-process-pool.png)

进程池实现：主进程维护一个子进程池（默认 4 个，与 CPU 核心数一致），任务按最少挂起数分发：
...
class CryptoProcessPool {
  ...
  spawnWorker() {
    const worker = fork("./crypto-worker.js"); // 子进程内通过 N-API 加载 .so
    worker.pending = 0;
  ...
  execute(action, payload) {
    ...
    return new Promise((resolve, reject) => {
      ...
      worker.send({ taskId: id, action, payload });
    });
  }
}
```
...
1. 内存泄漏：C++ 模块在异常路径下分配了内存但未释放。使用 valgrind 定位到问题，用 RAII 智能指针修复。
2. 段错误：传递了无效的 Buffer 指针，导致 C++ 模块访问非法内存。增加了参数校验和空指针检查；即使崩溃也只影响单个子进程。
3. ABI 不兼容：Node.js 和 C++ 模块使用了不同的 C++ 标准库版本。统一使用 libstdc++ 并设置 `-fPIC` 编译选项。
4. 并发安全：C++ 模块不是线程安全的，每个子进程内串行调用，多个子进程之间天然隔离，避免了数据竞争。

性能优化：对高频小任务做批量合并减少 IPC 次数，Buffer 复用降低分配开销。单次 IPC 开销约 1ms，相比计算耗时（加密 1MB 数据约 5ms，解析大表文件几十 ms）占比很小；4 个子进程并行处理，吞吐量提升约 3.5 倍。
...
问题现象：NoSQL 管理系统的 Node 进程运行 24 小时后，内存占用从启动时的 200MB 增长到 800MB 以上，且不会回落，最终 OOM。通过 heapdump 对比确认 JS 堆大小稳定，判断泄漏在 C++ 层。
...
==12345==    by 0x5A3F2E1: parse_table (parse_table.cpp:42)
==12345==    by 0x5A3F4A9: batch_parse (parse_table.cpp:87)
==12345==    by 0x10B8F2: Napi::Function::Call(...) (napi-inl.h:1234)
```

日志显示 `parse_table.cpp:42` 行分配了 1MB 内存但未释放。
...
int parse_table(const char* input, int input_len, char* output, int* output_len) {
    ...
    if (decrypt(input, input_len, temp_buffer) != 0) {
        return -1; // 泄漏点: 异常路径 early return, 跳过了 free
    }

    // ... 解析逻辑 ...
    *output_len = parsed_len;
...
正常路径有释放逻辑，但解密失败（表文件损坏、密钥错误）时的 early return 跳过了 `free`。根本性修复是使用 RAII（Resource Acquisition Is Initialization）模式，用智能指针自动管理内存，保证任何返回路径都会释放：
...
- 对象池优化：GC 暂停时间从平均 15ms 降至 5ms，吞吐量提升约 20%
- 综合效果：加解密高频场景下 API 响应时间明显下降
...
A: 字节 Data 部门的搜索推荐算法平台和抖音 Debug 平台每天产生大量 JSError，人工修复成本高。我参与设计了一套 LLM 自动修复系统，显著提升了修复效率。
...
- LLM 生成的 patch 通过自动验证（编译 + 单元测试）的比率约 65%
- 通过验证的 patch 经人工审核后合入率约 47%
- 高频模式化错误（如空值访问、依赖缺失）自动修复成功率最高，可达 70% 以上；复杂业务逻辑错误主要作为辅助参考
- 错误 MTTR（平均修复时间）从约 4 天降到 1 天以内，每周节省 on-call 人力约 6 小时
...
A: A2UI（Agent to UI）是字节内部开发的 AI 驱动 UI 生成框架，核心理念是通过自然语言描述生成 React 组件。我在字节 Data 部门和阿里妈妈广告技术部都参与了 A2UI 的接入工作。

A2UI 工作链路：自然语言描述经 LLM 解析生成 Component Schema（JSON），Schema 渲染器结合 Component Registry（预定义组件库）动态渲染出可交互的 React 组件。
...
架构设计：管理后台修改配置后，API Server 通过 Schema Generator 产出 JSON Schema 并下发；前端的 Schema Renderer 结合 Component Registry 解析 Schema，动态渲染出完整页面。
...
1. 修复 HMR 兼容性问题：Vite dev server 下远程模块变更后热更新不生效（模块没有被正确 invalidate），我分析了原因并提交 PR，在插件的 HMR 处理链路中补上了远程模块映射的失效与刷新逻辑。
2. 优化 shared 依赖的预构建逻辑：原实现对每个 shared 依赖单独做一次 optimizeDeps，依赖多时预构建耗时很长。我将多个 shared 依赖合并为一次 optimizeDeps 调用，将预构建时间从约 12 秒降低到 3 秒。
3. 添加 runtime plugin 机制：允许开发者在运行时动态修改远程模块的加载行为（如添加鉴权 header、切换 CDN 源等），对齐 Webpack 版本 Module Federation 的 runtime plugin API。
...
- SWR 策略：先返回缓存，后台更新（字节 Data 项目，首屏 3.2s → 1.4s）
- 多层缓存：L1 BFF 内存 + L2 Redis + L3 ClickHouse/MySQL（TikTok 项目，面板查询 P99 控制在 300ms 以内）
...
- 对象池：复用频繁创建/销毁的对象，GC 暂停从 15ms 降至 5ms（腾讯 IEG 项目）
