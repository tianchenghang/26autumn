...
A: 每个对象都有一个内部槽 [[Prototype]]，指向它的原型对象，可以通过 Object.getPrototypeOf 或 proto 访问。函数对象额外拥有一个 prototype 属性，该属性的 constructor 指回函数本身。当通过 new 创建实例时，实例的 [[Prototype]] 会被设为构造函数的 prototype。
...
常见追问：Object.create(null) 创建的对象没有原型，适合作为纯字典使用，避免 proto、constructor 等键名冲突；直接给 obj.proto 赋值会改变对象形状导致引擎去优化，应优先用 Object.create 或 class。
...
- class 声明不会提升（处于 TDZ），而函数声明会提升。
- class 内部代码自动运行在严格模式。
- class 的方法不可枚举（[[Enumerable]] 为 false），而直接给 prototype 赋值的方法默认可枚举。
- class 必须通过 new 调用，直接调用抛 TypeError。
- extends 做了两件事：设置子类 prototype 的原型为父类 prototype（实例继承），同时设置子类本身的原型为父类（静态成员继承），即 SubClass.proto === ParentClass。
...
- 并非所有事件都冒泡：focus/blur、mouseenter/mouseleave、load、error（资源加载）等不冒泡；对应的可冒泡替代是 focusin/focusout、mouseover/mouseout。不冒泡的事件仍可在捕获阶段被祖先监听。
