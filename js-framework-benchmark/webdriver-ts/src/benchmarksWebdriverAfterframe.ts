...
import { BenchmarkType, Benchmark, cpuBenchmarkInfos, CPUBenchmarkInfo } from "./benchmarksCommon.js";
...
export const benchRemove = new (class extends CPUBenchmarkWebdriver {
  ...
  async init(driver: WebDriver) {
    ...
    await testTextContains(
      driver,
      `//tbody/tr[${this.rowsToSkip + 1}]/td[1]`,
      `${this.rowsToSkip + this.benchmarkInfo.warmupCount + 1}`,
      config.TIMEOUT,
      false
    );
    ...
    await testTextContains(
      driver,
      `//tbody/tr[${this.rowsToSkip + 2}]/td[1]`,
