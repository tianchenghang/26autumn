  ...
  entries.forEach((x) => {
    ...
    if (e.name === "EventDispatch") {
      ...
      if (config.LOG_DETAILS)
        console.log("TimerFire", +e.ts, +e.ts - startLogicEvent_startTS, +e.ts - startLogicEvent_endTS);
      ...
      if (config.LOG_DETAILS)
        console.log("RequestAnimationFrame", +e.ts, +e.ts - startLogicEvent_startTS, +e.ts - startLogicEvent_endTS);
      filteredEvents.push({
...
export async function computeResultsCPU(
  ...
  let startFrom = R.filter(type_eq(startLogicEventName, "fireAnimationFrame", "timerFire", "layout", "functioncall"))(
...
export function computeResultsJS(cpuTrace: CPUDurationResult, config: Config, fileName: string): Promise<number> {
  return computeResultsFromTrace(cpuTrace, config, fileName, traceJSEventNames, true);
}

export function computeResultsPaint(cpuTrace: CPUDurationResult, config: Config, fileName: string): Promise<number> {
...
export async function computeResultsFromTrace(
  ...
  const eventsWithin = R.filter<TimingResult>((e) => e.ts >= totalDuration.tsStart && e.ts <= totalDuration.tsEnd)(
    perfLogEvents
  );
