...
async function runSizeBenchmark(
  ...
  try {
    ...
    for (let i = 0; i < benchmarkOptions.batchSize; i++) {
      let enableCompressionResponse = await fetch(
        `http://${benchmarkOptions.host}:${benchmarkOptions.port}/enableCompression`
      );
      ...
      let paintEvents = JSON.parse(
        (await page.evaluate(`JSON.stringify(performance.getEntriesByType("paint"))`)) as string
      );
    ...
    let disableCompressionResponse = await fetch(
      `http://${benchmarkOptions.host}:${benchmarkOptions.port}/disableCompression`
    );
...
export async function executeBenchmark(
  ...
  let runBenchmarks: Array<BenchmarkSize> = benchmarks.filter(
    (b) => benchmarkId === b.benchmarkInfo.id && b instanceof BenchmarkSize
